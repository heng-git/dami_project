...
func (con PassController) Login(c *gin.Context) {
	...
	c.HTML(http.StatusOK, "shopping/pass/login.html", gin.H{
		"prevPage": prevPage,
	})
}
func (con PassController) RegisterStep1(c *gin.Context) {
	c.HTML(http.StatusOK, "shopping/pass/register_step1.html", gin.H{})
}
func (con PassController) RegisterStep2(c *gin.Context) {
	...
	if len(userTemp) > 0 {
		c.HTML(http.StatusOK, "shopping/pass/register_step2.html", gin.H{
...
func (con PassController) RegisterStep3(c *gin.Context) {
	...
	models.DB.Where("sign=?", sign).Find(&userTemp)
	if len(userTemp) > 0 {
		c.HTML(http.StatusOK, "shopping/pass/register_step3.html", gin.H{
			"smsCode": smsCode,
			"sign":    sign,
		})
	} else {
...
func (con PassController) SendCode(c *gin.Context) {
	...
	*/
	...
	//4、判断当前ip地址今天发送短信的次数
	//每发送一条短信 user_temp表就存一次该短信相关
	ip := c.ClientIP()            //获取客户端的ip
	...
	userTemp := []models.UserTemp{}
	smsCode := models.GetRandomNum()       //即模拟的短信验证码
...
func (con PassController) LoginOut(c *gin.Context) {
	...
	prevPage := c.Request.Referer() //保存当前页面
	if len(prevPage) > 0 {
		c.Redirect(302, prevPage) //跳转到当前页面
	} else {
		c.Redirect(302, "/")
	}
}
