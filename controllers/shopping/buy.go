...
func (con BuyController) Checkout(c *gin.Context) {
	...
	//4、判断orderList数据是否存在
	if len(orderList) == 0 { //防止重复提交订单
		c.Redirect(302, "/")
		return
	}
	con.Render(c, "shopping/buy/checkout.html", gin.H{
		"orderList":   orderList,
		"allPrice":    allPrice,
		"allNum":      allNum,
...
func (con BuyController) DoCheckout(c *gin.Context) {
	...
	if orderSignClient != orderSignServer { //由于点击去支付之后就立即删除了该订单   因此可以防止重复提交
		c.Redirect(302, "/")
		return
	}
	...
	if len(addressResult) == 0 { //如果没有收货地址  直接返回checkout页面
...
func (con BuyController) Pay(c *gin.Context) {
	...
	con.Render(c, "shopping/buy/pay.html", gin.H{
		"order":      order,
