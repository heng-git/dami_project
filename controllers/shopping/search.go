...
func (con SearchController) Query(c *gin.Context) { //模糊查询
	...
	query := elastic.NewMatchQuery("Title", "手机")  //模糊查询  查询title中有小米字段的所有结果
	searchResult, err := models.EsClient.Search(). //下面的字段是查询条件
	...
	goods := models.Goods{}
	c.JSON(200, gin.H{
		"searchResult": searchResult.Each(reflect.TypeOf(goods)), //通过反射获取goods类型的数据
	})
}
