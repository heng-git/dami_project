...
func (con CartController) Get(c *gin.Context) {
	...
	var tpl = "shopping/cart/cart.html"
	...
	*/
	...
	if len(cartList) > 0 {
		...
		if models.HasCartData(cartList, currentData) {
			for i := 0; i < len(cartList); i++ {
				//如果cookie里面的数据和当前要加入的数据完全相同就直接将其数量+1
		...
		models.Cookie.Set(c, "cartList", cartList) //更新cartList
...
func (con CartController) AddCartSuccess(c *gin.Context) {
	...
	var tpl = "shopping/cart/addcart_success.html"
	con.Render(c, tpl, gin.H{
		"goods": goods,
	})
}
