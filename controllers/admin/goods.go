...
func (con GoodsController) Index(c *gin.Context) {
	//当前页数
	page, _ := models.Int(c.Query("page")) //如果c.Query请求为空  则默认传过来的page为0
	if page == 0 {
		page = 1
	}
	//获取keyword
	...
	//每页查询的数量
	pageSize := 5
	where := "is_delete=0" //设置查询条件
	if len(keyword) > 0 {
		where += " AND title like \"%" + keyword + "%\"" //由于外面是双引号 里面若想也用双引号 需要使用\转义
	}
	goodsList := []models.Goods{}
	models.DB.Where(where).Offset((page - 1) * pageSize).Limit(pageSize).Find(&goodsList) //goodsList为每页商品的数量
	...
	if len(goodsList) > 0 {
		...
		if page != 1 {
			c.Redirect(302, "/admin/goods") // 重定向跳转应该是302
		} else {
			c.HTML(http.StatusOK, "admin/goods/index.html", gin.H{
				"goodsList": goodsList,
				//注意float64类型
...
func (con GoodsController) Edit(c *gin.Context) {
	...
	goodsColorList := []models.GoodsColor{}
	models.DB.Find(&goodsColorList)
	for i := 0; i < len(goodsColorList); i++ { //由于渲染的时候需要把所有颜色显示出来  因此需要遍历所有颜色
	...
	c.HTML(http.StatusOK, "admin/goods/edit.html", gin.H{
		...
		"prevPage":       c.Request.Referer(), //获取点击修改时的页面地址
	})
}
func (con GoodsController) DoEdit(c *gin.Context) {
	...
	//6、修改规格包装  1、删除当前商品下面的规格包装   2、重新执行增加 （重新增加更粗暴一点）
...
func (con GoodsController) EditorImageUpload(c *gin.Context) {
	...
	if err != nil {
		...
		if models.GetOssStatus() != 1 { //如果是本地裁剪
			wg.Add(1)
			go func() {
			...
			c.JSON(http.StatusOK, gin.H{
				"link": "/" + imgDir, //加个 “/”能保证前面的域名不会变（localhost:8080）
			})
		} else {
			//fmt.Println(models.GetSettingFromColumn("OssDomain") + imgDir)
			c.JSON(http.StatusOK, gin.H{
...
func (con GoodsController) GoodsImageUpload(c *gin.Context) {
	...
	if err != nil {
		...
		if models.GetOssStatus() != 1 { //将图片保存在本地
			wg.Add(1)
			go func() {
				models.ResizeGoodsImage(imgDir)
				wg.Done()
